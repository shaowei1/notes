...
from core.utils import delete_empty_data
from engine.configs import (
    AccessKeyID,
    AccessKeySecret,
    oss_host,
    oss_callback_url,
    oss_token_expire_time,
    oss_bucket_name,
    oss_endpoint,
    PRODUCE
)
...
if PRODUCE is None:
    oss_token_expire_time = 60 * 60 * 1024
...
# 确认上面的参数都填写正确了
for param in (AccessKeyID, AccessKeySecret, oss_bucket_name, oss_endpoint):
...
def build_signature(AccessKeySecret, encode_policy):
    """生成签名
    :param str AccessKeySecret: access key secret
    :param str encode_policy: 编码后的Policy
    :return str 请求签名
    """
    h = hmac.new(AccessKeySecret.encode(), encode_policy, hashlib.sha1)
...
def build_post_url(oss_endpoint, oss_bucket_name):
    """生成POST请求URL
    :param str oss_endpoint: oss_endpoint
    :param str oss_bucket_name: bucket name
    :return str POST请求URL
    """
    if oss_endpoint.startswith('http://'):
        return oss_endpoint.replace('http://', 'http://{0}.'.format(oss_bucket_name))
    elif oss_endpoint.startswith('https://'):
        return oss_endpoint.replace('https://', 'https://{0}.'.format(oss_bucket_name))
    else:
        return 'http://{0}.{1}'.format(oss_bucket_name, oss_endpoint)
...
def get_token(key, min_content_length=None, max_content_length=None, data=None):
    """
    ...
    """
    callback_body_list = ['bucket=${bucket}',
                          'object=${object}',
                          'etag=${etag}',
                          'size=${size}',
                          'mimeType=${mimeType}',
    ...
    field_dict['key'] = key
    # access key id
    field_dict['OSSAccessKeyId'] = AccessKeyID
    # Policy包括超时时间(单位秒)和限制条件condition
    field_dict['policy'] = build_encode_policy(
        oss_token_expire_time,
        [
            ['eq', '$bucket', oss_bucket_name],
            ['content-length-range', min_content_length, max_content_length]
        ]
    )
    print(f"initial oss_bucket_name {oss_bucket_name}")
    print(f"initial oss_endpoint {oss_endpoint}")
    print(f"initial oss_host {oss_host}")
    print(f"initial AccessKeyID {AccessKeyID}")

    # 请求签名
    field_dict['Signature'] = build_signature(AccessKeySecret, field_dict['policy'])

    logging.info(f"customize data: {data}")

    if data is not None:
        new_data = delete_empty_data(data)
        key = 'customize'
        # customize_data = '{' + ','.join([':'.join([f'"{str(k)}"', f'"{str(v)}"']) for k, v in new_data.items()]) + '}'
        customize_data = json.dumps(new_data)
        # callback中的自定义变量，没有回调不填该域
        field_dict[f'x:{key}'] = customize_data
        callback_body_list.append('%s=${x:%s}' % (key, key))

    callback_body = '&'.join(callback_body_list)

    # callback，没有回调需求不填该域
    field_dict['callback'] = bulid_callback(
        f'{oss_callback_url}',
        callback_body,
        'application/json'
    )  # 'filename=${object}&size=${size}&mimeType=${mimeType}', # 'application/x-www-form-urlencoded')

    for key, value in field_dict.items():
        if isinstance(value, bytes):
            field_dict[key] = value.decode()
