...
#### middle table
```
statement = category_size_guide_chart.delete().where(
    category_size_guide_chart.c.size_guide_chart_id == size_chart_id)
mysql_store.session.execute(statement)

statement = mysql_store.session.query(user_tp.c.tp_id).filter(user_tp.c.user_id == user_id).all()

_p = mysql_store.session.query(Product.category_id).filter(Product.id == product_id).subquery('_p')
_m = mysql_store.session.query(CategoryMapping.tp_category_id) \
    .filter(CategoryMapping.category_id == _p.c.category_id, CategoryMapping.tp_id == tp_id).subquery('_m')
_tp_category_props = mysql_store.session.query(TpCategoryProp) \
    .options(joinedload(TpCategoryProp.sub_props)) \
    .filter(TpCategoryProp.tp_category_id == _m.c.tp_category_id, TpCategoryProp.parent_id.is_(None)) \
    .order_by(TpCategoryProp.sort_order.asc()).all()
...
https://marshmallow.readthedocs.io/en/stable/s
```
class CategorySizeGuideChart(db.Model):
    __tablename__ = 'category_size_guide_chart'

    category_id = Column(INTEGER(10), primary_key=True, nullable=False)
    size_guide_chart_id = Column(ForeignKey('size_guide_chart.id', ondelete='CASCADE'), primary_key=True,
                                 nullable=False, index=True)
    path = Column(String(255))

    size_guide_chart = relationship('SizeGuideChart', backref=db.backref('categories', cascade="all, delete-orphan"))
...
image = relationship("Image",
                     foreign_keys=[id],
                     # order_by=lambda: desc(Image.created_at),
                     primaryjoin="and_(File.type=='image',File.id==Image.id)",
                     uselist=False,
...
sub_files = relationship('File',
                        primaryjoin=remote(parent_id) == foreign(
                            id),
                        uselist=True,
                        # backref=db.backref('parent'), # 不要轻易使用
                        cascade='all,delete-orphan',
                        single_parent=True
                        #  delete-orphan cascade is not supported on a many-to-many or many-to-one relationship when single_parent is not se
                        )
